...
cp $rpath/SubProcesses/MadLoop.mg5 . >& /dev/null
...
cp -r $rpath/Source/DHELAS . >& /dev/null
cp -r $rpath/Source/MODEL . >& /dev/null
cp -r $rpath/SubProcesses Madlib >& /dev/null

rm -f Madlib/coupl.inc >& /dev/null

ln MODEL/coupl.inc Madlib/  >& /dev/null
