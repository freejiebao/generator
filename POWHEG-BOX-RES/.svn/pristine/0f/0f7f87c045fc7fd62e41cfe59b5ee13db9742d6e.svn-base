...
c
      ...
      implicit none
      character *(*) fname
      call pwhgloadhistos_rename(fname,' ',' ')
      end

      subroutine pwhgloadhistos_rename(fname,prefix,suffix)
c     reads a file with output histograms and reloads them
c     appendig the prefix and the suffix at the front and end of each histogram name.
      implicit none
      character *(*) fname,prefix,suffix
...
c     of the string is returned.
            ...
            string=trim(adjustl(prefix))//trim(string)//trim(adjustl(suffix))
c count the lines
            k=0
            read(10,'(a)') line
            do while(line.ne.' ')
     ...
     2               +(hist_ptr(ind2)%errhistarr2(1,k)/hist_ptr(ind2)%yhistarr2(1,k))**2)
      ...
      elseif(op.eq.'+') then
         do k=1,hist_ptr(ind1)%nbins
            hist_ptr(ind3)%yhistarr2(1,k) =  
     1           hist_ptr(ind1)%yhistarr2(1,k) + hist_ptr(ind2)%yhistarr2(1,k) 
            hist_ptr(ind3)%errhistarr2(1,k) = 
     1           sqrt((hist_ptr(ind1)%errhistarr2(1,k))**2
     2               +(hist_ptr(ind2)%errhistarr2(1,k))**2)
         enddo
      elseif(op.eq.'ave') then
         do k=1,hist_ptr(ind1)%nbins
            hist_ptr(ind3)%yhistarr2(1,k) =  
     1           (hist_ptr(ind1)%yhistarr2(1,k) + hist_ptr(ind2)%yhistarr2(1,k))/2
            hist_ptr(ind3)%errhistarr2(1,k) = 
     1           sqrt((hist_ptr(ind1)%errhistarr2(1,k))**2
     2               +(hist_ptr(ind2)%errhistarr2(1,k))**2)/2
         enddo
