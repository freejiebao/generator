...
module openloops
  ...
  public :: evaluate_tree, evaluate_cc, evaluate_ccmatrix, evaluate_sc
  public :: evaluate_cc2, evaluate_ccmatrix2, evaluate_sc2
  public :: evaluate_loopcc
  public :: evaluate_scpowheg, evaluate_scpowheg2
  public :: evaluate_tree_colvect, evaluate_tree_colvect2
  public :: evaluate_full, evaluate_loop, evaluate_loop2, evaluate_loop2ir, evaluate_ct, evaluate_pt
  ! Print parameters
  ...
  function register_process_string(process_in, amptype)
    ...
    integer, allocatable :: pol(:)
    integer, allocatable :: extid(:)
    integer :: coupling_QCD_0_bak, coupling_EW_0_bak
    ...
    if (index(process_in, ">") > 0) then
      ...
      if (amptype == 99 .or. write_shopping_list ) then ! write shopping list
        ...
        if (register_process_string > 0 .and. add_associated_ew > 0 .and. coupling_EW(1) == 0 ) then
        ...
        ! register associate QCD-EW born amplitude
        if (register_process_string > 0 .and. add_associated_ew > 1) then
          coupling_EW_0_bak  = coupling_EW(0)
          coupling_QCD_0_bak = coupling_QCD(0)
          if (coupling_EW(1) == 0) then
            call set_parameter("coupling_qcd_0",-1)
            call set_parameter("coupling_ew_0",coupling_EW_0_bak+1)
          else if (coupling_QCD(1) == 0) then
            call set_parameter("coupling_ew_0",-1)
            call set_parameter("coupling_qcd_0",coupling_QCD_0_bak-1)
          end if
          associated_born = loop_over_libraries(proc, 1, n_in, perm, pol, extid, process_in)
          process_handles(register_process_string)%associated_born = associated_born
          call set_parameter("coupling_qcd_0",coupling_QCD_0_bak)
          call set_parameter("coupling_ew_0",coupling_EW_0_bak)
        end if
  ...
  ! loop over library types
    use KIND_TYPES, only: DREALKIND
    use ol_parameters_decl_/**/DREALKIND, only: coupling_QCD, coupling_EW
    implicit none
    ...
    if (check < 1) then ! not found anywhere
      call ol_msg("register_process: process " // trim(process_in) // &
          & " EW=" // trim(to_string(coupling_EW(0)))  // "," // &
          & trim(to_string(coupling_EW(1))) // &
          & " QCD=" // trim(to_string(coupling_QCD(0)))  // "," // &
          & trim(to_string(coupling_QCD(1))) // &
          & " not found!")
    end if
  ...
  subroutine evaluate_cc2(id, psp, res, cc, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    if (error > 1) return
    subprocess = process_handles(id)
    if (.not. btest(subprocess%content, 1) .and. .not. btest(subprocess%content, 1)) then
  ...
  subroutine evaluate_full(id, psp, m2l0, m2l1, ir1, m2l2, ir2, acc)
    ...
    if (add_associated_ew > 1 .and. subprocess%associated_born > 0) then
      call evaluate_tree(subprocess%associated_born,psp,m2l0ew)
      m2l1(0) = m2l1(0)+m2l0ew
    else if (add_associated_ew > 1 .and. subprocess%associated_born <= 0) then
  ...
  subroutine evaluate_fullcr(id, psp, m2l0, m2l1, ir1, m2l2, ir2, cr, acc)
    ...
    if (any(subprocess%last_psp /= psp) .or. &
      ...
      if (subprocess%last_zero) return ! return for unstable points
    end if
    if (cr%type < 1) then
      return
  ...
  subroutine evaluate_loop2ir(id, psp, res, resir, acc)
    use ol_generic, only: to_string
    implicit none
    integer, intent(in)  :: id
    real(DREALKIND), intent(in)  :: psp(:,:)
    ...
    resir(1) = ir2(1)
    resir(2) = ir2(2)
  end subroutine evaluate_loop2ir


  subroutine evaluate_loop2ir_c(id, pp, res, resir, acc) bind(c,name="ol_evaluate_loop2ir")
    implicit none
    integer(c_int), value :: id
    real(c_double), intent(in) :: pp(5*n_external(id))
    ...
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
    call evaluate_loop2ir(f_id, f_pp(0:3,:), f_res, f_resir, f_acc)
    res = f_res
    resir = f_resir
    acc  = f_acc
  end subroutine evaluate_loop2ir_c
