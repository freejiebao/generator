...
c coupling rescaling, for born (imode=1) and NLO corrections (imode=2)
     ...
     1     savnlofac(maxprocborn),savmuf2(maxprocborn)
      ...
      integer flav_fs(nlegborn),nlegs_fs
      real * 8 p_fs(0:3,nlegborn)

      call get_fs_particles_momenta(flst_bornlength(iuborn),
     $     flst_born(:,iuborn),flst_bornres(:,iuborn),kn_cmpborn,
     $     nlegs_fs,flav_fs,p_fs)

      do j=1,nlegs_fs
         do mu=0,3
            if(op(mu,j).ne.p_fs(mu,j)) then
               do k=1,flst_nborn
                  valid(k)=.false.
               enddo
               op=p_fs
      ...
      if(valid(iuborn)) then
         ...
         flav=flav_fs
         call setlocalscales0(nlegs_fs,flav,p_fs,
...
c If the virtual is not included, it must be omitted.            
     ...
     1     (nlegs,flav,pin,basicfac,bornfac,nlofac)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_st.h'
      include 'pwhg_math.h'
      integer nlegs,flav(nlegs)
      real * 8 pin(0:3,nlegs),basicfac,bornfac,nlofac
      integer onem
     ...
     1     lscalej,lscalek
      ...
      q2mergeMAX=-1d10
      do l=1,nlegs
         call findNearestNeighbours(nlegs,p,lflav,jmerge,kmerge,mergedfl,
     $                              q2merge)
...
C ------------------------------------------------ C
      subroutine findNearestNeighbours(nlegs,p,lflav,jmerge,kmerge,mergedfl,
     $                                 q2merge)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_st.h'
