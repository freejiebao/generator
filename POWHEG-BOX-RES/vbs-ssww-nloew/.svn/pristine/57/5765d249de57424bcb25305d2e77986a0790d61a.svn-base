...
c.....mauro
     ...
     +     mc_dlberad
      integer check,checkf
      logical fsrrad,isrrad
c
      fsrrad=.false.
      isrrad=.false.
...
c the emitter is always withing the original flst_ibornlength position
               ...
               if(nlegr.gt.nnnb) then
                  write(*,*)'gen_leshouches: nlegr>nnnb'
     =                 //',should not be here!'
                  write(*,*) ' exiting ...'
                  call exit(-1)
               endif
...
c.....mauro-mod                           
                     ...
                     check =check +1
                     checkf=checkf+1
                     mc_fsr_scale(checkf)=sqrt(t)
                     mc_fsr_em(   checkf)=em
                     fsrrad=.true.
c.....mauro-mod                           
                  endif
c                  write(*,*)'em   ',em,'nlegr',nlegr,'sqrt(t)',sqrt(t)
c                  write(*,*)'reslistreal',reslistreal(nlegr)
c                  write(*,*)'flst_alr   ',flst_alr(1:nlegr,alr)
c                  write(*,*)'check',check
               else
                  scalup=sqrt(t)
               endif
...
!     if(check.ne.1.and.check.ne.2.and.check.ne.0) then
c      if(check.eq.3) then
c         write(*,*)'evento con due radiazioni '
cc         stop
c      endif
      if(  check.ne.1 .and. check.ne.2 .and. check.ne.3.and.
     +     check.ne.4 .and. check.ne.0) then
         write(*,*) ' gen_leshouches: error, 5 scales not ok...'
         write(*,*) ' exiting  ...'
         call exit(-1)
      endif
      if(flg_allrad) then
c defaults now decided in lhefread !
c         if(.not.isrrad) mc_isr_scale=sqrt(rad_ptsqmin)
c         if(.not.fsrrad) mc_fsr_scale=sqrt(rad_ptsqmin_em)
         if(isrrad.and.fsrrad) mc_dlberad=.true.
      endif
