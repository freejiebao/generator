      ...
      real * 8 kt2minqed
      real * 8 xphcut
      real * 8 vetoscale_isr, vetoscale_fsr,massres
      integer evtnumber,emitter_fsr,nfsres
      common/si_event_info/xphcut,vetoscale_isr,vetoscale_fsr(3),evtnumber,emitter_fsr(3),nfsres,massres(3)
...
c Initialize Fortran based analysis (histograms)
      ...
      if (no_tworadevents) then
         write(*,*)'option not alllowed for this process'
         stop
         write (*,*) '*** SI: Using only scalup from LHE'//
     +        ' events as starting scale for radiation'
      endif
