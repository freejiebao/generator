...
c  pwhgfill  :  fills the histograms with data
      ...
      call bookupeqbins('totalcut',1d0,-0.5d0,10.5d0)
      call bookupeqbins('ptgama',1d0,0d0,500d0)

c      call bookupeqbins('Nphot',1d0,-0.5d0,5.5d0)

      call bookupeqbins('V_pt',1.d0,0d0,300d0)
      call bookupeqbins('V_y',0.2d0,-4d0,4d0)
      call bookupeqbins('V_m',1.d0,60d0,120d0)

      call bookupeqbins('l_pt',1.d0,0d0,300d0)
      call bookupeqbins('l_y',0.2d0,-4d0,4d0)
      call bookupeqbins('l_eta',0.2d0,-2.5d0,2.5d0)
      call bookupeqbins('l_m',1d0,0d0,100d0)
      ...
      real * 8 pw(4),pl(4),pnu(4),pr(4),ptrad,y,eta,pt,m
      ...
      ptrad = sqrt(pr(1)**2+pr(2)**2)

      call filld('totalcut',0d0,dsig)
      if(getpt(pl).gt.10) then
         call filld('totalcut',1d0,dsig)
      endif
      call filld('ptgama',ptrad,dsig)
      if(ptrad.gt.3) then
         call filld('totalcut',2d0,dsig)
      endif
...
c lepton 1
      ...
      call filld('l_pt', pt, dsig)
      call filld('l_m', m, dsig)
c lepton 2
      call getyetaptmass(pnu,y,eta,pt,m)
      call filld('nu_y',    y, dsig)
      call filld('nu_eta',eta, dsig)
      call filld('nu_pt', pt, dsig)
c vector
      call getyetaptmass(pnu+pl,y,eta,pt,m)
