...
if __name__ == "__main__":
    ...
    for ipar,param in enumerate(params):
        ...
        with open('submit_{0}.jdl'.format(int_process),'w') as outfile:
            ...
            outfile.write("Queue\n")
        os.system("curl http://stash.osgconnect.net/+jiexiao/wrapper.sh -o wrapper_{0}.sh".format(int_process))
        ...
        with open('submit_{0}.jdl'.format(bsm_process),'w') as outfile:
            ...
            outfile.write("when_to_transfer_output = ON_EXIT\n")
            outfile.write("Queue\n")
        os.system("curl http://stash.osgconnect.net/+jiexiao/wrapper.sh -o wrapper_{0}.sh".format(bsm_process))
        ...
        for jpar in range(ipar+1,len(params)):
            ...
            with open('submit_{0}.jdl'.format(d2_process),'w') as outfile:
                ...
                outfile.write("when_to_transfer_output = ON_EXIT\n")
                outfile.write("Queue\n")
            os.system("curl http://stash.osgconnect.net/+jiexiao/wrapper.sh -o wrapper_{0}.sh".format(d2_process))
